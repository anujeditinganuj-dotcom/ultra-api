...
def resolve():
    ...
    if html:
        _nd_url_match = re.search(
            r'"(?:url|permalink|slug|canonicalUrl)"\s*:\s*"([^"]*videos/[^"]+)"',
            html
        )
        if _nd_url_match:
            _nd_url = _nd_url_match.group(1).replace("\\u002F", "/").replace("\/", "/")
            if len(_nd_url) > len(filename_url):  # longer = has full slug
                filename_url = _nd_url if _nd_url.startswith("http") else f"https://faphouse.com{_nd_url}"
    ...
    slug_title = _slug_from_url(filename_url)   # always English
    ...
    # Title priority: URL slug > yt-dlp > scraped (localized)
    # Slug is most reliable for English title — it's part of the URL and never localized
    english_title = slug_title or ytdlp_title or meta["title"] or "Unknown"
