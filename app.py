...
def api_download():
    ...
    Returns the mp4 as a streamed attachment — ffmpeg's output is piped
    straight to the HTTP response as it's produced, so the browser starts
    receiving (and showing download progress for) the file within a
    second or two, instead of waiting for the whole video to finish
    downloading and remuxing on the server first.
    """
    ...
    cmd = [
        ...
        "-i", m3u8_url,
        "-c", "copy",
        # FIX: was writing to a NamedTemporaryFile on disk with
        # subprocess.run() blocking until the ENTIRE video (these run
        # several GB — see the "7.44 GB" example earlier) had fully
        # downloaded and remuxed, THEN calling send_file() once that was
        # done. Three things went wrong with that:
        #  1. Render's own edge proxy has a request timeout well under
        #     the time a multi-GB mux can take, so the connection got
        #     killed before send_file() ever ran — the browser just saw
        #     the request hang forever with no error, matching "Preparing…
        #     shows, download never starts".
        #  2. Render's free-tier disk is small and ephemeral; a handful of
        #     concurrent multi-GB downloads can fill it before ffmpeg even
        #     finishes.
        #  3. "+faststart" needs a second pass to seek back and move the
        #     moov atom to the front — impossible on a live pipe, which is
        #     exactly why this used a temp file in the first place.
    ...
    # Explicitly NOT setting Content-Length — it isn't known ahead of time
    # for a live-piped, fragmented-mp4 stream. Browsers handle a
    # Content-Length-less attachment download fine (chunked transfer).
    return resp
