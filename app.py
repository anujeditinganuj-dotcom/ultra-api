...
def api_download():
    """
    Redirect straight to the CDN's m3u8 URL instead of proxying video
    bytes through this server.

    Query params:
      url      — FapHouse video page URL
      quality  — optional, e.g. "720p" (must match a label from
                 /api/resolve's available_qualities). Omitted or "auto"
                 uses whatever get_m3u8_url() resolves (same stream the
                 "Stream (Auto Best)" button opens).

    CHANGED: this used to spawn ffmpeg on the server, remux the HLS
    stream into a single mp4, and pipe that to the browser — all of the
    actual video bandwidth flowed through Render. Now the server only
    resolves the CDN URL and 302s; the browser fetches segments directly
    from video-pr.xhcdn.com, same as clicking the Stream link. Render's
    CPU/bandwidth is no longer in the path at all.

    Trade-off: the CDN URL is an .m3u8 playlist, not a single .mp4 file,
    and the CDN doesn't send a Content-Disposition: attachment header.
    Browsers therefore won't auto-save a video file the way the old
    ffmpeg version did — most will just open/display the playlist text
    (Safari, which supports HLS natively, will usually play it). An
    actual saved mp4 now needs a client-side HLS-aware downloader (a
    browser extension, IDM, yt-dlp, etc.) pointed at this URL, since the
    server no longer does that remuxing.
    """
    ...
    try:
        ...
        if quality and quality.lower() != "auto":
            ...
            match = next((q for q in qualities if q["label"].lower() == quality.lower()), None)
