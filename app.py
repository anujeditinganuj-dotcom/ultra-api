...
def api_download():
    ...
    cmd = [
        "ffmpeg", "-y",
        "-rw_timeout", "30000000",      # 30s connection timeout (was 20s)
        "-reconnect", "1",
        "-reconnect_streamed", "1",
        "-reconnect_delay_max", "10",   # retry up to 10s between attempts (was 5s)
        "-reconnect_on_network_error", "1",
        "-timeout", "30000000",         # per-segment read timeout 30s
    ...
    def generate():
        import select
        # Stall timeout: how long to wait with ZERO bytes before giving up.
        # 25s was too aggressive for large files (2-10GB) on slow CDN links
        # — a legitimate 10GB download at 5 MB/s takes 2000s total, and the
        # CDN can pause naturally between segments without it being a stall.
        # 120s gives enough room for slow segments while still catching a
        # genuinely stuck connection.
        STALL_TIMEOUT = 120  # seconds with zero bytes before giving up
        try:
            while True:
                ...
                if not ready:
                    ...
                    proc.kill()
                    break
                chunk = proc.stdout.read(1024 * 512)  # 512KB chunks for large files
                if not chunk:
                    break
                yield chunk
        finally:
            proc.stdout.close()
            try:
                proc.wait(timeout=30)  # give ffmpeg time to flush on large files
    ...
    # Do NOT set Content-Length — estimated size causes browsers to mark
    # the download as "incomplete" when the real file size differs from
    # the estimate (which it always does for HLS streams). Without
    # Content-Length, browsers download until the stream ends naturally.
    return resp
