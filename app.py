...
(function(){
    ...
    fetch('/api/faphouse?url=' + encodeURIComponent(url))
      ...
      .then(function(d){
        ...
        if(data.m3u8_link){
          ...
          html += '<button class="dl-btn" onclick="startDownload(\'' + encodeURIComponent(lastFetchedUrl) + '\'))">⬇ Download</button>';
        }

        lastFetchedUrl = url;   // store for Download button
  ...
  // ── Download handler ─────────────────────────────────────────────
  window.startDownload = function(encodedUrl){
...
def api_download():
    """
    Stream the Auto (Best) m3u8 video directly to the browser as an mp4 download.

    Query params:
      url  — FapHouse video page URL

    Returns the mp4 file as an attachment so the browser shows a Save-As dialog.
    Uses ffmpeg to mux the HLS stream into a single mp4 file in a temp dir,
    then streams it back with the correct Content-Disposition header.
    """
    ...
    try:
        m3u8_url = faphouse.client.get_m3u8_url(url)
        if not m3u8_url:
            return jsonify({"error": "Could not resolve m3u8 URL"}), 502

        # Get a clean filename from page meta
        meta = {}
        try:
        ...
        if proc.returncode != 0:
            os.unlink(tmp_path)
            return jsonify({"error": "ffmpeg failed", "detail": proc.stderr.decode()[-500:]}), 500
        ...
        return jsonify({"error": "Download timed out (video too large or slow connection)"}), 504
    except Exception as e:
        logger.exception(f"api_download error: {e}")
...
def resolve():
    ...
    # FIX: Previously checked _is_mostly_english() FIRST and skipped translation
    # if >85% chars were ASCII. But Spanish/Italian/French titles like
    # "estado de ánimo viajero" are ~96% ASCII and passed that check — so
    # they went straight to english_title without ever being translated.
    #
    # New strategy: ALWAYS try translation first.
    # deep-translator returns the original text unchanged when it's already
    # English, so this is a no-op for English videos. Non-English titles
    # (even ASCII-heavy ones like Spanish/Italian) now get properly translated.
    if meta["title"]:
        translated = _translate_to_english(meta["title"])
        if translated and translated.strip():
            english_title = translated
            if translated.lower().strip() != (meta["title"] or "").lower().strip():
                logger.info(f"[translate] '{(meta['title'] or '')[:50]}' → '{translated[:50]}'")
        else:
            # Translation failed / timed out — best-effort fallback
