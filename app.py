...
def api_download():
    ...
    Query params:
      ...
      quality  — optional, e.g. "1080p" / "480p" (must match a label from
                 /api/resolve's available_qualities). Defaults to "720p"
    ...
    # Default quality is now 720p (not "auto/best") — per your last request.
    # Pass ?quality=1080p / ?quality=480p / etc. to override, or
    # ?quality=auto to explicitly get the old best-available behavior.
    quality = request.args.get("quality", "").strip() or "720p"
    ...
    try:
        ...
        if quality.lower() != "auto":
            ...
            if not matched_q and qualities:
                # This exact video doesn't have a 720p rendition (some don't).
                ...
                # what was requested, so the result stays as close to 720p
