...
def resolve():
    ...
    # Pehle sequential tha (6-17s). Ab teeno ek saath chalte hain.
    # Total time = slowest single call (~2-4s) instead of sum of all.
    ...
    def _task_qualities():
        return faphouse.get_available_qualities(url) or []

    with ThreadPoolExecutor(max_workers=3) as pool:
        fut_html = pool.submit(_task_html)
        fut_m3u8 = pool.submit(_task_m3u8)
        fut_qual = pool.submit(_task_qualities)

        # HTML result
        try:
            results["html"], results["final_url"] = fut_html.result(timeout=15)
        except Exception as e:
            logger.warning(f"HTML fetch failed: {e}")

        # M3U8 result
        try:
            results["m3u8"] = fut_m3u8.result(timeout=15)
        ...
        # Qualities result
        try:
            results["qualities"] = fut_qual.result(timeout=15)
        except faphouse.FanclubLockedError as e:
            results["fanclub_err"] = str(e)
        except Exception as e:
    ...
    if quality_list:
        ...
        for q in quality_list:
            if q.get("label") and q.get("url"):
                # _fix_url() still applies here — unrelated to the
                # Markdown-link removal below, this is about the CDN
                # links themselves containing literal unescaped commas
                # (`key=...,s=,end=...`) that some clients' auto-link
                # detection truncates at, same reasoning as every other
                # URL emitted in this function.
    ...
    speed_link = next(
        ...
        m3u8_link  # fallback to master m3u8 if no direct quality URLs
    )
    ...
    # Basic info first, then links/URLs, then formatted_text last as a
    ...
    data["thumbnail"] = _e("", _fix_url(meta.get("thumbnail")))
    data["speed_link"] = _e("", _fix_url(speed_link))
    data["m3u8_link"] = _e("", _fix_url(m3u8_link))
    data["available_qualities"] = [
        {
            "label": _quality_label(q.get("label", "")),
            "url":   _e("", _fix_url(q.get("url"))),
        }
        for q in (quality_list or [])
        if q.get("url")
    ]
    data["formatted_text"] = _e("", formatted_text)
