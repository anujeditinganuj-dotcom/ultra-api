...
def resolve():
    ...
    if _existing_ytdlp:
        # yt-dlp ALWAYS returns original English title — use it as primary source,
        # overriding whatever the page scraper got (which may be localized).
        ytdlp_title = _existing_ytdlp.get("title")
        if ytdlp_title:
            meta["title"] = ytdlp_title  # always prefer yt-dlp title
        if not meta["description"]:
            meta["description"] = _existing_ytdlp.get("description")
    # Title: use yt-dlp title (most accurate, matches video's natural language)
    # If yt-dlp fails, use scraped title, then URL slug as last fallback
    slug_title    = _slug_from_url(filename_url)
    english_title = meta["title"] or slug_title
