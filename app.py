...
def resolve():
    ...
    # Prefer URL slug (always English) if scraped title looks non-English.
    # FapHouse localizes og:title / __NEXT_DATA__ title based on visitor IP/locale.
    # The URL slug is always the original English title — use it as primary source.
    slug_title = _slug_from_url(filename_url)
    scraped    = meta["title"] or ""

    def _is_likely_english(text: str) -> bool:
        if not text:
            return False
        # If >30% chars are outside ASCII printable range → likely non-English
        non_ascii = sum(1 for c in text if ord(c) > 127)
        if non_ascii / max(len(text), 1) > 0.2:
            return False
        return True

    if slug_title and not _is_likely_english(scraped):
        english_title = slug_title
    else:
        english_title = scraped or slug_title
