...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  /* ── Search Box ── */
  .search-box{ margin:28px 0 0; }
  .search-row{ display:flex; gap:8px; align-items:center; }
  .search-input{
    flex:1; background:var(--panel); border:1px solid var(--line); border-radius:8px;
    padding:10px 14px; font-family:'JetBrains Mono',monospace; font-size:0.85rem;
  ...
  .search-result-header{
    ...
    font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--faint);
    display:flex; align-items:center; justify-content:space-between; gap:8px;
  }
  .status-ok{ color:var(--teal); }
  .status-err{ color:var(--rose); }
  .search-result-body{ padding:14px; }
  .result-thumb{ width:100%; max-height:160px; object-fit:cover; border-radius:6px; margin-bottom:10px; display:none; }
  .result-thumb.show{ display:block; }
  .result-title{ font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:0.95rem; color:var(--text); margin-bottom:8px; }
  .result-meta{ display:flex; flex-wrap:wrap; gap:6px; margin-bottom:10px; }
  .result-meta-item{ font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--faint); background:var(--panel-2); padding:3px 8px; border-radius:4px; border:1px solid var(--line); }
  .result-qualities{ display:flex; flex-wrap:wrap; gap:6px; margin-bottom:10px; }
  .quality-btn{ background:transparent; border:1px solid var(--line); border-radius:6px; padding:5px 12px; font-family:'JetBrains Mono',monospace; font-size:0.78rem; color:var(--muted); cursor:pointer; text-decoration:none; transition:all .15s; display:inline-block; }
  .quality-btn:hover{ border-color:var(--amber); color:var(--amber); background:rgba(242,166,65,.05); }
  .stream-btn{ display:inline-flex; align-items:center; gap:6px; background:var(--amber); color:#1a1206; border:none; border-radius:7px; padding:8px 16px; font-family:'Inter',sans-serif; font-weight:600; font-size:0.82rem; text-decoration:none; cursor:pointer; transition:background .15s, transform .15s; }
  .stream-btn:hover{ background:#f5b45f; transform:translateY(-1px); }
  .result-error{ color:var(--rose); font-family:'JetBrains Mono',monospace; font-size:0.82rem; }
  .search-loading{ display:none; color:var(--faint); font-family:'JetBrains Mono',monospace; font-size:0.8rem; padding:8px 0; }
  ...
  <div class="search-box">
    <div class="search-row">
      <input class="search-input" id="searchInput" type="text"
        placeholder="https://faphouse.com/videos/... or faphouse2.com/videos/..."
    ...
    <div class="search-result" id="searchResult">
      <div class="search-result-header">
        ...
        <span id="resultUrl" style="max-width:60%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></span>
      </div>
      <div class="search-result-body" id="resultBody"></div>
    </div>
  </div>
...
(function(){
  ...
  var rStatus = document.getElementById('resultStatus');
  var rUrl    = document.getElementById('resultUrl');
  var rBody   = document.getElementById('resultBody');
  ...
  function doFetch() {
    ...
    if (!url.includes('faphouse')) {
      ...
      rStatus.innerHTML = '<span class="status-err">&#x274C; Not a FapHouse link</span>';
      rUrl.textContent = '';
      rBody.innerHTML = '<div class="result-error">Enter a faphouse.com or faphouse2.com video URL.</div>';
      return;
    }
    ...
    fetch('/api/faphouse?url=' + encodeURIComponent(url))
      ...
      .then(function(d){
        ...
        if (!d.status) {
          rStatus.innerHTML = '<span class="status-err">&#x274C; Error</span>';
          rUrl.textContent = '';
          rBody.innerHTML = '<div class="result-error">' + (d.message || 'Unknown error') + '</div>';
          return;
        }

        var data = d.data;
        rStatus.innerHTML = '<span class="status-ok">&#x2705; OK</span>';
        rUrl.textContent = url;
        var html = '';

        var keys = Object.keys(data);
        var thumbKey = keys.find(function(k){ return k.includes('thumbnail'); });
        var thumb = thumbKey ? data[thumbKey] : null;
        if (thumb) html += '<img class="result-thumb show" src="' + thumb + '" onerror="this.style.display=\'none\'">';

        var titleKey = keys.find(function(k){ return k.includes('title'); });
        var title = titleKey ? (data[titleKey] || '').replace(/^\S+\s/, '') : '';
        if (title) html += '<div class="result-title">' + title + '</div>';

        var meta = '';
        var authorKey = keys.find(function(k){ return k.includes('author'); });
        var durKey    = keys.find(function(k){ return k.includes('duration') && !k.includes('seconds'); });
        var sizeKey   = keys.find(function(k){ return k.includes('size') && !k.includes('bytes'); });
        if (authorKey && data[authorKey]) meta += '<span class="result-meta-item">' + data[authorKey] + '</span>';
        if (durKey    && data[durKey])    meta += '<span class="result-meta-item">' + data[durKey] + '</span>';
        if (sizeKey   && data[sizeKey])   meta += '<span class="result-meta-item">' + data[sizeKey] + '</span>';
        if (meta) html += '<div class="result-meta">' + meta + '</div>';

        var qKey = keys.find(function(k){ return k.includes('qualities') || k.includes('quality'); });
        var qualities = qKey ? (data[qKey] || []) : [];
        if (qualities.length) {
          html += '<div class="result-qualities">';
          qualities.forEach(function(q) {
            var qkeys = Object.keys(q);
            var urlKey = qkeys.find(function(k){ return k.includes('url'); });
            var qurl = urlKey ? q[urlKey] : '';
            var label = (q.label || '').replace(/[^\w\s]/g, '').trim();
            if (qurl) html += '<a class="quality-btn" href="' + qurl + '" target="_blank">' + label + '</a>';
          });
          html += '</div>';
        }

        var m3uKey = keys.find(function(k){ return k.includes('m3u8'); });
        var m3u8 = m3uKey ? data[m3uKey] : null;
        if (m3u8) html += '<a class="stream-btn" href="' + m3u8 + '" target="_blank">&#x25B6; Stream (Auto Best)</a>';
      ...
      .catch(function(e){
        ...
        btn.disabled = false; btn.textContent = 'Fetch';
        result.classList.add('visible');
        rStatus.innerHTML = '<span class="status-err">&#x274C; Network Error</span>';
        rBody.innerHTML = '<div class="result-error">' + e.message + '</div>';
      });
  }
