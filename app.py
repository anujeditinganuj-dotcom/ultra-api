...
<div class="page">
  ...
  <div class="header-row">
    <div class="reel">
      <!-- Crown logo matching FapHouse brand -->
      <svg width="44" height="38" viewBox="0 0 44 38" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M4 34 L4 14 L12 24 L22 4 L32 24 L40 14 L40 34 Z" fill="#f2a641" stroke="none"/>
        <circle cx="4" cy="13" r="3.5" fill="#f2a641"/>
        <circle cx="22" cy="3.5" r="3.5" fill="#f2a641"/>
        <circle cx="40" cy="13" r="3.5" fill="#f2a641"/>
        <!-- arch cutout -->
        <path d="M17 34 L17 26 Q17 22 22 22 Q27 22 27 26 L27 34 Z" fill="#0d0f14"/>
      </svg>
      <h1 style="font-size:2rem;letter-spacing:-0.02em;">
        <span style="color:#f2a641;">fap</span><span style="color:#fff;">house</span>
...
(function(){
  ...
  function syntaxJSON(obj){
    ...
    return s
      ...
      .replace(/:\s*"([^"]*)"/g, function(m,v){ return ': <span class="js">"' + v + '"</span>'; })
  ...
  function doFetch(){
    ...
    fetch('/api/faphouse?url=' + encodeURIComponent(url))
      ...
      .then(function(d){
        ...
        if(d.status){
          ...
          if(copyBtn) copyBtn.addEventListener('click', function(){
            navigator.clipboard.writeText(JSON.stringify(d, null, 2)).then(function(){
              ...
              copyBtn.innerHTML = '<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="2,7 5,10 12,3"/></svg>Copied';
              setTimeout(function(){ copyBtn.classList.remove('copied'); copyBtn.innerHTML = '<svg viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="4" y="4" width="8" height="8" rx="1.5"/><path d="M2 10V2h8"/></svg>Copy JSON'; }, 1800);
            });
          });
        } else {
          respContent.innerHTML =
        ...
        if(data.thumbnail)
          html += '<img class="video-thumb" src="' + data.thumbnail + '" onerror="this.style.display=\'none\'">';
        ...
        if(quals.length){
          ...
          quals.forEach(function(q){
            if(q.url) html += '<a class="quality-btn" href="' + q.url + '" target="_blank">' + (q.label||'') + '</a>';
          });
          html += '</div>';
        }

        if(data.m3u8_link)
          html += '<a class="m3u8-btn" href="' + data.m3u8_link + '" target="_blank">▶ Stream (Auto Best)</a>';
  ...
  function renderLib(){
    ...
    list.innerHTML = lib.map(function(item, i){
      var thumb = item.thumbnail
        ? '<img class="lib-icon" src="' + item.thumbnail + '" onerror="this.style.display=\'none\'">'
