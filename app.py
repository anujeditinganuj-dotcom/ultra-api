  ...
  function doFetch(){
    ...
    fetch('/api/faphouse?url=' + encodeURIComponent(url))
      ...
      .then(function(d){
        ...
        // BUG FIX: escape title before injecting into innerHTML to prevent XSS
        // (a video title containing </div><script>... would execute as JS).
        function escHtml(s){
          return String(s)
            .replace(/&/g,'&amp;').replace(/</g,'&lt;')
            .replace(/>/g,'&gt;').replace(/"/g,'&quot;');
        ...
        if(data.m3u8_link){
          ...
          // BUG FIX: extra closing paren ')' in onclick — was '...url)\'))'
          // which produced startDownload('url')) — a syntax error in the
          // browser (unexpected token ')' after the function call expression).
  ...
  function renderLib(){
    ...
    list.innerHTML = lib.map(function(item, i){
      ...
      return '<div class="lib-item" data-idx="' + i + '">' +
        thumb +
        '<div class="lib-info">' +
          '<div class="lib-name">' + (item.title || item.url).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') + '</div>' +
