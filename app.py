...
<style>
  ...
  .theme-btn{
    position:fixed; top:16px; right:74px; z-index:50;
  ...
  *{ box-sizing:border-box; }
  ...
  .lib-btn-count{
    ...
    min-width:20px; height:20px; padding:0 4px; font-size:0.7rem; font-weight:700;
...
<div class="page">
  ...
  <div class="header-row">
    <div class="reel">
      <!-- Ultra lightning bolt logo -->
      <svg width="38" height="42" viewBox="0 0 38 42" fill="none" xmlns="http://www.w3.org/2000/svg">
        <polygon points="22,2 6,24 18,24 14,40 32,16 20,16" fill="#f2a641" stroke="#f2a641" stroke-linejoin="round"/>
      </svg>
...
<span class="kw">else</span>:
  ...
  <div style="margin-top:52px;border-top:1px solid var(--line);padding-top:32px;">
    <div style="display:flex;flex-wrap:wrap;gap:40px;margin-bottom:32px;">

      <div>
        <p style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;color:var(--muted);text-transform:uppercase;margin:0 0 14px;">Contact</p>
        <a href="https://t.me/anujedits97" target="_blank" rel="noopener"
           style="display:inline-flex;align-items:center;gap:8px;color:var(--text);text-decoration:none;font-size:0.9rem;background:var(--card);border:1px solid var(--line);border-radius:8px;padding:8px 14px;transition:border-color .2s;">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="#2AABEE"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.562 8.248l-2.018 9.51c-.145.658-.537.818-1.084.508l-3-2.21-1.447 1.394c-.16.16-.295.295-.605.295l.213-3.053 5.56-5.023c.242-.213-.054-.333-.373-.12L7.26 14.42l-2.95-.924c-.642-.2-.655-.643.136-.953l11.527-4.448c.535-.194 1.003.13.59.153z"/></svg>
          @anujedits97
        </a>
      </div>

      <div>
        <p style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;color:var(--muted);text-transform:uppercase;margin:0 0 14px;">Support</p>
        <a href="https://t.me/anujedits97" target="_blank" rel="noopener"
           style="color:var(--muted);text-decoration:none;font-size:0.88rem;display:block;margin-bottom:8px;">Bug Report</a>
        <a href="https://t.me/anujedits97" target="_blank" rel="noopener"
           style="color:var(--muted);text-decoration:none;font-size:0.88rem;display:block;">Feature Request</a>
      </div>

    </div>

    <footer>
      <p>Ultra API &mdash; FapHouse Resolver &mdash; <a href="/health">health</a></p>
      <p id="uptime-txt" style="color:var(--faint);font-size:0.8rem;"></p>
    </footer>
    <p style="color:var(--faint);font-size:0.75rem;margin-top:10px;">&copy; 2026 Ultra API. All rights reserved.</p>
  </div>
...
def resolve():
    ...
    # Title priority:
    # 1. Scraped title if already English
    # 2. Translate scraped title if non-English (slug may also be non-English)
    # 3. URL slug as last resort
    # 4. "Unknown"
    if meta["title"] and _is_mostly_english(meta["title"]):
        english_title = meta["title"]
    else:
        # Try translating the scraped title
        translated = _translate_to_english(meta["title"])
        if translated and _is_mostly_english(translated):
            english_title = translated
            logger.info(f"[translate] '{(meta['title'] or '')[:40]}' → '{translated[:40]}'")
        elif slug_title and _is_mostly_english(slug_title):
            english_title = slug_title
        elif slug_title:
            # Translate slug too if it's non-English
            translated_slug = _translate_to_english(slug_title)
            english_title = translated_slug or slug_title or meta["title"] or "Unknown"
        else:
            english_title = meta["title"] or "Unknown"
    ...
    if meta.get("duration"):
        flines.append(f"⏱️ *Duration:* {meta['duration']}")
    if size_human:
    ...
    if quality_list:
        ...
        url_q = _fix_url(best.get("url", ""))
        if label and url_q:
            flines.append(f"🎞️ *Quality:* [{label}]({url_q})")
