...
(function(){
  ...
  // ── Download handler ─────────────────────────────────────────────
  // FIX: pehle encodedUrl (encodeURIComponent) leti thi — ab plain URL
  // directly milta hai (closure se), decodeURIComponent ki zaroorat nahi.
  window.startDownload = function(videoUrl){
    ...
    fetch('/api/download?url=' + encodeURIComponent(url))
      .then(function(r){
        if(!r.ok){
          // Try JSON error body, fallback to status text
        ...
        var filename = fnMatch ? decodeURIComponent(fnMatch[1].trim()) : 'video.mp4';
...
def api_download():
    ...
    """
    import tempfile, subprocess, mimetypes, shutil
    ...
    try:
        ...
        try:
            # Dynamic Referer based on video domain
            ...
            # Safety net: if response was not sent, clean up now
