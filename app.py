...
(function(){
  ...
  window._libAdd = addToLib;
    ...
    # auto-generated URL for a fresh deploy. request.url_root reflects
    # whatever host+scheme the page is actually being *viewed* through
    # right now, so it's automatically correct everywhere: a VPS's own
    # domain/IP, a fresh Render deploy's *.onrender.com URL, behind a
    # reverse proxy — no env var or redeploy-time config needed either way.
