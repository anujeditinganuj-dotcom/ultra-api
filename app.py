...
def resolve_faphouse():
    ...
    return jsonify({
        ...
        "data": {
            ...
            # DIRECT M3U8 - No tokens exposed to user
            "direct_stream": m3u8 or (qualities[0].get("url") if qualities else None),
            # BEST QUALITY M3U8
            "best_quality_m3u8": next((q.get("url") for q in qualities if q.get("url")), m3u8),
        },
    })
        ...
        "qualities": [
            {
                ...
                # DIRECT M3U8 - Clean URL without extra wrapper
                "direct_m3u8":  q.get("url"),
            }
            for q in qualities
        ],
    })
...
def download_video():
    ...
    # DOWNLOAD-SPECIFIC: Use longer timeout and proper streaming
    referer = faphouse.get_base_url(url) or "https://faphouse.com"
    
    # FFmpeg command with LONGER timeout for downloads (5 minutes instead of 20 seconds)
    cmd = [
        "ffmpeg", "-y",
        "-rw_timeout", "300000000",  # 5 minutes for downloads (vs 20s for streaming)
        "-reconnect", "1",
        "-reconnect_streamed", "1",
        "-reconnect_delay_max", "10",  # Longer delay for downloads
        "-headers", f"Referer: {referer}/\r\nUser-Agent: Mozilla/5.0\r\n",
        "-i", stream_url,
        "-c", "copy",
        "-movflags", "frag_keyframe+empty_moov+faststart",
    ...
    logger.info(f"[ffmpeg-download] {filename}")
    try:
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:
        return _error("ffmpeg not installed on this server.", 500)
    ...
    def generate_download():
        sent_bytes = 0
        try:
            while chunk := proc.stdout.read(262144):  # Larger chunks (256KB) for downloads
                sent_bytes += len(chunk)
                yield chunk
        except Exception as e:
            ...
            if proc.returncode != 0:
                logger.error(
                    f"[ffmpeg-download] {filename} FAILED (exit {proc.returncode}) after "
                    f"{sent_bytes} bytes sent — last output:\n" + "\n".join(stderr_tail)
                )
            elif sent_bytes < 1000000:  # Less than 1MB suggests incomplete
                logger.warning(
                    f"[ffmpeg-download] {filename} completed but only {sent_bytes} bytes sent"
                )
            else:
                logger.info(f"[ffmpeg-download] {filename} successfully completed ({sent_bytes} bytes)")

    return Response(
        stream_with_context(generate_download()),
        headers={
...
# ── HLS Proxy routes ────────────────────────────────────────────────────────
# FapHouse HLS segments are served from CDN URLs that are only reachable via
# an authenticated session (cookies/headers the browser doesn't have).
# These two routes tunnel them through the server's authenticated session so
# any HLS player can stream video without needing faphouse credentials.
...
def proxy_m3u8():
    ...
    url = unquote(url)
    try:
        content = faphouse.client.fetch_m3u8_proxied(url)
        if content:
...
def proxy_media():
    ...
    return faphouse.client.fetch_media_proxied(url, range_header)


@app.route("/api/info")
def get_info_clean():
    """
    CLEAN RESPONSE: Direct m3u8 and download URLs without API wrapper.
    
    GET /api/info?url=<faphouse_link>
    
    Response example:
    {
        "status": true,
        "title": "Video Title",
        "stream_url": "https://video-pr.xhcdn.com/key=xxx/.../_TPL_.mp4.m3u8",
        "download_url": "https://best-cdn.faphouse.com/download-endpoint",
        "thumbnail": "https://thumbnail-url",
        "size": "1.2 GB",
        "qualities": [
            {"label": "1080p", "m3u8": "https://...m3u8"},
            {"label": "720p", "m3u8": "https://...m3u8"}
        ]
    }
    """
    url = (request.args.get("url") or "").strip()
    ...
    try:
        meta = faphouse.get_metadata(url)
    except faphouse.FanclubLockedError as e:
        return _error(f"Fanclub locked: {e}", 403)
    except Exception as e:
        return _error(f"Metadata fetch failed: {e}", 502)
    ...
    # Build response with direct URLs (no token exposure in response format)
    title = meta.get("title")
    name = _sanitize_filename(title) if title else _slug_filename(url)
    
    best_quality = next((q for q in (qualities or []) if q.get("url")), None)
    stream_url = best_quality.get("url") if best_quality else m3u8
    
    size_bytes, size_human = _probe_size(stream_url) if stream_url else (None, None)
    
    qualities_list = [
        {"label": q.get("label", "Unknown"), "m3u8": q.get("url")}
        for q in (qualities or [])
        if q.get("url")
    ]
    ...
    return jsonify({
        ...
        "size": size_human,
        "size_bytes": size_bytes,
        "stream_url": stream_url,  # Direct m3u8 - Can be used directly!
        "download_url": stream_url,  # Same as stream for video players
