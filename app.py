...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  .lib-btn-count{
    ...
    min-width:13px; height:13px; padding:0 2px; font-size:0.55rem; font-weight:700;
    display:none; align-items:center; justify-content:center;
    /* BUG FIX: was var(--bg) — that custom property was never defined
       anywhere (:root only defines --ink for the page background), so
       this border-color failed to resolve and the badge lost its
       "cutout ring" against the button behind it — most visible in
       light mode where the mismatch stood out most. */
    position:absolute; top:-4px; right:-4px; border:2px solid var(--ink);
  }
  .lib-btn-count.show{ display:flex; }
  /* BUG FIX: this used to animate `transform: translateY(...)` too. Any
     element with a transform-affecting animation becomes a "containing
     block" for its own position:fixed descendants (per the CSS spec) —
     since .page wraps the whole page including .theme-toggle-wrap and
     .lib-btn (both position:fixed, meant to stay glued to the actual
     screen's top-right corner), that turned them into position:absolute-
     like elements relative to .page's own box instead, which is why they
     showed up sitting down in the content flow (right under the
     "FAPHOUSE" title) rather than pinned to the true top-right of the
     viewport. Keeping the fade but dropping the slide removes that
     containing-block side effect entirely. */
  ...
  .header-row{ display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:6px; padding-right:100px; }
...
<div class="page">
  ...
  <div class="header-row">
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:-30px;">
      <button class="theme-toggle-wrap" id="themeBtn" title="Toggle theme">
        <span class="toggle-icon" id="toggleIcon">🌙</span>
      </button>
      <button class="lib-btn" id="libBtn" title="Library">
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M2 4H14" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
          <path d="M2 8H14" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
          <path d="M2 12H14" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>
        </svg>
  ...
  <div class="site-footer">
    ...
    <div class="footer-bottom">
      <div class="footer-bottom-left">
        ...
        <!-- BUG FIX: .footer-health and #uptime-txt had CSS rules and a
             JS listener (getElementById('uptime-txt') in the /health
             ping below) with no matching HTML element at all — dropped
             during an earlier footer redesign and never restored. Net
             effect: the health-check link vanished from the footer
             entirely, and the live "Response time: Nms" text could
             never appear since there was nothing for that JS to fill in. -->
        <a class="footer-health" href="/health" target="_blank" rel="noopener">health</a>
...
def api_download():
    ...
    cmd = [
        ...
        # ADDED: the four flags above only cover an outright connection
        # drop — they don't help when the CDN keeps the TCP connection
        # open but just stops sending data mid-segment (a stall, not a
        # disconnect), which is what "download starts fine, then the
        # video pauses partway through" originally looked like.
        # reconnect_on_network_error covers a mid-segment read error
        # instead of only a failed initial connection.
        # NOTE: reconnect_delay_total was tried here too but this ffmpeg
        # build doesn't recognize it ("Unrecognized option") — that made
        # EVERY download fail immediately instead of just not fixing the
        # stall, which is strictly worse, so it's removed rather than
        # guessed at again. reconnect_delay_max (already set above)
        # still bounds the per-attempt retry delay, just not the total.
        # NOTE 2: reconnect_at_eof was also tried and REMOVED — production
        # logs showed it made things worse, not better: "Will reconnect
        # at 72293 in 0 second(s), error=End of file" at a suspiciously
        # small byte offset (the playlist/first-segment fetch, not deep
        # into the video), followed by total silence until the 25s
        # watchdog killed it. FapHouse's CDN links are signed/time-
        # limited — reconnect_at_eof's byte-offset resume apparently
        # isn't honored on them, so the "reconnect" just hangs forever
        # instead of ever getting new data, turning what used to be "it
        # pauses partway" into "it never starts at all". Removing it
        # goes back to just letting reconnect/reconnect_streamed handle
        # actual drops, which is what was confirmed working before.
        "-reconnect_on_network_error", "1",
        "-headers", f"Referer: {referer}\r\nUser-Agent: Mozilla/5.0\r\n",
        "-i", stream_url,
        "-c", "copy",
        # HLS delivers AAC audio in raw ADTS framing, which mp4's own
        # container format can't store as-is — muxing it straight through
        # with -c copy failed every time ("Malformed AAC bitstream
        # detected" / "Error submitting a packet to the muxer: Operation
        # not permitted", confirmed in production logs). This filter just
        # reframes ADTS into the length-prefixed format mp4 requires; it
        # doesn't touch the actual audio data, so there's no re-encode
        # and no quality loss, same as -c copy for video.
        "-bsf:a", "aac_adtstoasc",
    ...
    def generate():
        import select
        # Safety net on top of the reconnect flags above: those handle
        # ffmpeg retrying at the network-protocol level, but if the CDN
        # is stalled badly enough that even those retries don't produce
        # any output for a long stretch, proc.stdout.read() below can
        # block indefinitely with the process technically still "alive"
        # — that's exactly what showed up as a download frozen at a
        # fixed byte count with no error and no progress. select() lets
        # us wait for data with a real timeout instead of blocking
        # forever, so a truly-stuck stream fails cleanly (browser sees
        # the download stop with an incomplete file) rather than hanging
        # the connection open forever with no way to tell "still working"
        # apart from "permanently stuck".
        STALL_TIMEOUT = 25  # seconds with zero bytes before giving up
        try:
            while True:
                ready, _, _ = select.select([proc.stdout], [], [], STALL_TIMEOUT)
                if not ready:
                    logger.warning(f"api_download: no data for {STALL_TIMEOUT}s, "
                                    f"CDN stream appears stalled — aborting for {url}")
                    proc.kill()
                    break
