...
def resolve():
    ...
    def _translate_to_english(text: str) -> str | None:
        ...
        Tries deep-translator first, then googletrans as fallback.
        Both hit Google's free endpoint internally.
        4s timeout. Returns None on failure."""
        ...
        def _run():
            ...
            if _DEEP_TRANSLATOR_OK:
                try:
                    result["out"] = _DeepGoogleTranslator(
            ...
            if _GOOGLETRANS_OK:
                try:
                    t = _GoogletransTranslator()
                    result["out"] = t.translate(text, dest="en").text
                except Exception as e:
    ...
    # 2. URL slug (always English, but may be truncated)
    # 3. Google Translate (if not English)
    # 4. Original title as-is
    if meta["title"] and _is_mostly_english(meta["title"]):
        english_title = meta["title"]
    elif slug_title:
        english_title = slug_title
    else:
        # Non-English title — translate it
        translated = _translate_to_english(meta["title"])
        english_title = translated or meta["title"] or "Unknown"
        if translated:
            logger.info(f"[translate] '{meta['title'][:40]}' → '{translated[:40]}'")
