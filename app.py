...
def resolve():
    ...
    # ── Title: yt-dlp se English title (most reliable) ───────────────
    # yt-dlp FapHouse extractor always returns English title.
    # HTML scraping se jo title aata hai wo localized ho sakta hai.
    # URL slug fallback: agar yt-dlp bhi fail kare.
    ytdlp_title = _ytdlp_title(url)
    english_title = ytdlp_title or _slug_from_url(filename_url) or meta["title"]

    # ── Filename ──────────────────────────────────────────────────────
    filename = _make_filename(english_title or meta["title"], filename_url)
