...
def api_download():
        ...
        def generate():
            ...
            try:
                while True:
                    ...
                    if not ready:
                        logger.warning(f"api_download: stalled {STALL_TIMEOUT}s — aborting")
                        proc.kill()
                        break
                ...
                if rc not in (0, None, -9):
                    ...
                    logger.warning(f"api_download: exit {rc}: {stderr_tail}")
        return generate

    # ── Strategy 1: yt-dlp (primary) ──────────────────────────────────
    # yt-dlp handles HLS segments natively with automatic retry —
    # no mid-segment stalls, no ffmpeg pipe bottleneck.
    ytdlp_bin = shutil.which("yt-dlp")
    if ytdlp_bin:
        height = re.sub(r"[^0-9]", "", quality) or "480"
        fmt = ("bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best"
               if quality.lower() == "auto"
               else f"bestvideo[height<={height}][ext=mp4]+bestaudio[ext=m4a]/best[height<={height}]/best")
        ytdlp_cmd = [
            ytdlp_bin,
            "--no-warnings", "--quiet",
            "--add-header", f"Referer:{referer}",
            "--add-header", "User-Agent:Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/124",
            "--retries", "15",
            "--fragment-retries", "15",
            "--retry-sleep", "fragment:exp=1:max=8",
            "--no-part",
        ...
        try:
            ...
            logger.info(f"api_download: yt-dlp started for {url}")
            resp = Response(stream_with_context(_make_generate(proc)()), mimetype="video/mp4")
            resp.headers["Content-Disposition"] = f'attachment; filename="{out_filename}"'
            return resp
        except Exception as e:
            logger.warning(f"api_download: yt-dlp failed ({e}), trying ffmpeg")

    # ── Strategy 2: ffmpeg (fallback) ──────────────────────────────────
    if not shutil.which("ffmpeg"):
        return jsonify({"error": "Neither yt-dlp nor ffmpeg found on server"}), 503
    ...
    try:
        ...
        logger.info(f"api_download: ffmpeg started for {url}")
    except Exception as e:
