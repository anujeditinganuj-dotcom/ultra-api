...
def resolve():
    ...
    # 2. yt-dlp title — usually English but can time out (12s)
    # 3. Scraped title — may be localized (Hindi/Indonesian etc.) based on server IP

    slug_title = _slug_from_url(filename_url)   # always English, but only ever as long as the URL slug itself

    # Run yt-dlp in background (non-blocking) for description only
    _existing_ytdlp = _ytdlp_extract_info(url)
    ytdlp_title = (_existing_ytdlp or {}).get("title") if _existing_ytdlp else None
    if not meta["description"] and _existing_ytdlp:
    ...
    # Title priority: real scraped/yt-dlp title (full, correctly cased) if
    # it's actually in English > slug (always English, but only ever as
    # long/complete as the URL slug — e.g. "slutty milf bhabhi maan" for
    # a URL whose real title is a full sentence) > whichever real title
    # exists even if not confirmed English, so this is never "Unknown"
    # just because the language check was inconclusive.
    if meta["title"] and _is_mostly_english(meta["title"]):
        english_title = meta["title"]
    elif ytdlp_title and _is_mostly_english(ytdlp_title):
        english_title = ytdlp_title
    else:
        english_title = slug_title or meta["title"] or ytdlp_title or "Unknown"
    ...
    def _e(emoji, val):
        """Add emoji prefix to value if not None."""
        return f"{emoji} {val}" if val else None

    def _quality_emoji(label: str) -> str:
        """Return quality emoji based on resolution label."""
        l = (label or "").lower()
    ...
    data: dict = {
        ...
        "available_qualities": [
            {
                "label": _quality_emoji(q.get("label", "")),
                "url":   _e("🔗", q.get("url")),
            }
        ...
        "m3u8_link":           _e("📺",   m3u8_link),
        "size":                _e("📦",   size_human),
        "size_bytes":          _e("💾",   size_bytes),
        "speed_link":          _e("⚡",   speed_link),
        "thumbnail":           _e("🖼️",  meta["thumbnail"]),
